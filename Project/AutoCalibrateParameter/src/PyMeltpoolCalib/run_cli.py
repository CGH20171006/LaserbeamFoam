...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--method",
        type=str,
        choices=["acbici", "bayes", "gradient"],
        default="bayes",
        help="优化方法: acbici (MCMC), bayes (贝叶斯优化), gradient (梯度优化) (默认: bayes)",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    # ACBICI 特定参数
    acbici_group = parser.add_argument_group("ACBICI 参数")
    acbici_group.add_argument(
        "--nsteps",
        type=int,
        default=None,
        help="MCMC 步数 (默认: 500)",
    ...
    acbici_group.add_argument(
        "--kernel",
        type=str,
        choices=["expo", "matern32", "matern52", "ratquad"],
        default=None,
        help="GP 核函数 (默认: matern32)",
    )
    acbici_group.add_argument(
        "--synthetic-data",
        type=str,
...
def create_config(args: argparse.Namespace):
    """根据命令行参数创建配置对象"""
    from PyMeltpoolCalib.config import ACBICIConfig, BayesConfig, GradientConfig
    ...
    # 根据方法选择配置类
    if args.method == "acbici":
        if config_path.exists():
            config = ACBICIConfig.from_yaml(config_path)
        else:
            # 使用默认配置
            config = ACBICIConfig(
                ...
                synthetic_data_file=base_dir / "synthetic_data.npz",
            )
        ...
        if args.nwalkers is not None:
            config.nwalkers = args.nwalkers
        if args.burn is not None:
            config.burn = args.burn
        if args.kernel is not None:
            config.kernel = args.kernel
        if args.synthetic_data is not None:
            config.synthetic_data_file = Path(args.synthetic_data)
...
def run_acbici(config, exp_data: np.ndarray, verbose: bool = False):
    """运行 ACBICI 优化"""
    from PyMeltpoolCalib.optimizers import ACBICIOptimizer

    print("\n" + "=" * 60)
    print("ACBICI 优化 (MCMC 后验分布采样)")
    print("=" * 60)
    print(f"MCMC 步数: {config.nsteps}")
    print(f"Walkers: {config.nwalkers}")
    print(f"Burn-in: {config.burn * 100:.0f}%")
    print(f"核函数: {config.kernel}")
    print()

    optimizer = ACBICIOptimizer(config)
...
def run_bayes(config, exp_data: np.ndarray, verbose: bool = False):
    """运行贝叶斯优化"""
    from PyMeltpoolCalib.optimizers import BayesOptimizer
    from PyMeltpoolCalib.simulation import SimulationRunner
...
def run_gradient(config, exp_data: np.ndarray, verbose: bool = False):
    """运行梯度优化"""
    from PyMeltpoolCalib.optimizers import GradientOptimizer
    from PyMeltpoolCalib.simulation import SimulationRunner
...
def print_result(result, param_names: list = None):
    ...
    # ACBICI 特定输出
    if result.posterior_stats is not None:
        print("\n后验统计:")
        for name, stats in result.posterior_stats.items():
            print(f"  {name}:")
            print(f"    均值: {stats['mean']:.6g}")
            print(f"    标准差: {stats['std']:.6g}")
            print(f"    95% CI: [{stats['ci_lower']:.6g}, {stats['ci_upper']:.6g}]")
...
def main():
    ...
    # 加载实验数据
    from PyMeltpoolCalib.data import load_experiment_data
    ...
    try:
        if args.method == "acbici":
            result = run_acbici(config, exp_data, args.verbose)
        elif args.method == "bayes":
    ...
    # 生成对比图
    try:
        from PyMeltpoolCalib.postprocess import (
