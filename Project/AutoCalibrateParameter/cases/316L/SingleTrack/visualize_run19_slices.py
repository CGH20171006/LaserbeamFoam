...
"""
Multi-slice visualization for run 19.
...
def process_case_multi_slice(case_dir, output_dir, y_positions,
                             field='meltHistory', time_val=0.00034):
    """Load case once, then sweep through all Y slices."""
    ...
    # Slice filter (will update Origin for each Y position)
    ...
    for y in y_positions:
        y_um = int(round(y * 1e6))  # Convert to micrometers for filename

        # Move slice
        slice_filter.SliceType.Origin = [0.0, y, 0.0]
        slice_filter.UpdatePipeline()

        # Update camera focal point to slice
...
def main():
    ...
    # Y positions: 0 to 800um, every 40um
    y_positions = [i * 0.00004 for i in range(21)]  # 0, 40, 80, ..., 800 um

    print("=" * 80)
    print("Run 19 - Multi-Slice Visualization")
    print("=" * 80)
